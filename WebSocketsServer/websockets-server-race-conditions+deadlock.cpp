...
/*	
	...
	If we remove the Mutex, we can reliably reproduce the race condition on every run. (isMutexEnabled = FALSE)
	
	Here the code in on_client_connected has been refactored to create a deadlock situation. The server which acquires the Mutex and blocks on recv
	will block the other server threads.
*/
